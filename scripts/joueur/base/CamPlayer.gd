...
@export_range(-600.0, 0.0, 1.0) var look_ahead_grille_px: float = 0.0
...
@export_range(0.1, 30.0, 0.1) var offset_speed: float = 13.0

@export_group("Grille")
@export_range(0.0, 1.0, 0.01) var intensite_deplacement_grille: float = 0.0
@export_range(0.1, 30.0, 0.1) var lissage_vitesse_camera: float = 8.0
...
var _vitesse_cible_lissee: Vector2 = Vector2.ZERO
var _vp: Viewport
...
func _process(dt: float) -> void:
	if dt <= 0.0 or Engine.is_editor_hint():
		return
	...
	if target != _last_target:
		...
		_has_last = false
		_vitesse_cible_lissee = Vector2.ZERO
		_offset_balancement = Vector2.ZERO
	var in_drive: bool = _joueur.est_en_conduite()
	var mode_grille: bool = _est_en_mode_grille()
	...
	var look_ahead: float = look_ahead_sol_px
	if in_drive:
		look_ahead = look_ahead_conduite_px
	elif mode_grille:
		look_ahead = look_ahead_grille_px
	var mouse_look: float = mouse_look_conduite_px if in_drive else mouse_look_sol_px
	var intensite_balancement: float = intensite_vitesse
	if mode_grille:
		intensite_balancement *= intensite_deplacement_grille
	var desired_offset: Vector2 = dir * look_ahead + mouse_norm * mouse_look
	desired_offset += _calculer_offset_balancement(dir, intensite_balancement, dt)
	...
	if _shake_t > 0.0:
		...
		var tt: float = _ts()
		shake_off = Vector2(sin((tt + _shake_seed) * TAU * shake_freq_hz) * amp, cos((tt + _shake_seed) * TAU * shake_freq_hz * 0.93) * amp)
	...
	if not _zoom_moving:
		if zoom_out_delay_s <= 0.0 or _zoom_out_t >= zoom_out_delay_s:
			_zoom_moving = true
			_zoom_in_t = 0.0
	elif zoom_in_delay_s <= 0.0 or _zoom_in_t >= zoom_in_delay_s:
		_zoom_moving = false
		_zoom_out_t = 0.0
...
func _get_target_velocity(target: Node2D, dt: float) -> Vector2:
	var position_actuelle: Vector2 = target.global_position
	if not _has_last:
		_last_target_pos = position_actuelle
		_has_last = true
		_vitesse_cible_lissee = Vector2.ZERO
		return Vector2.ZERO
	var vitesse_brute: Vector2 = (position_actuelle - _last_target_pos) / maxf(dt, 0.00001)
	_last_target_pos = position_actuelle
	var lissage: float = 1.0 - exp(-lissage_vitesse_camera * dt)
	_vitesse_cible_lissee = _vitesse_cible_lissee.lerp(vitesse_brute, lissage)
	return _vitesse_cible_lissee

func _get_mouse_norm() -> Vector2:
	if _vp == null or _vp_center == Vector2.ZERO:
		return Vector2.ZERO
	var d: Vector2 = _vp.get_mouse_position() - _vp_center
	return Vector2(clampf(d.x * _vp_inv_center.x, -1.0, 1.0), clampf(d.y * _vp_inv_center.y, -1.0, 1.0))
...
func _est_en_mode_grille() -> bool:
	if _joueur == null or _joueur.gestion_deplacement == null:
		return false
	return _joueur.gestion_deplacement.est_type_grille_actif()
