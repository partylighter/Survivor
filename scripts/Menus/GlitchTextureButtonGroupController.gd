...
func _ready() -> void:
	...
	if _mat == null:
		push_error("GlitchTextureButtonController: mets le ShaderMaterial sur TextureButton.material")
		return
	...
	if not mouse_entered.is_connected(_on_enter):
		mouse_entered.connect(_on_enter, Object.CONNECT_DEFERRED)
	if not mouse_exited.is_connected(_on_exit):
		mouse_exited.connect(_on_exit, Object.CONNECT_DEFERRED)
	if not button_down.is_connected(_on_down):
		button_down.connect(_on_down, Object.CONNECT_DEFERRED)
	if not button_up.is_connected(_on_up):
		button_up.connect(_on_up, Object.CONNECT_DEFERRED)
...
func _apply_transform_state(s: int, dur: float) -> void:
	...
	var from_scale: Vector2 = scale
	var from_rot: float = rotation
	...
	_tw_xform.tween_method(func(v: float) -> void:
		var k: float = clamp(v, 0.0, 1.0)
		scale = from_scale.lerp(target_scale, k)
		rotation = lerp(from_rot, target_rot, k)
		position = from_pos.lerp(target_pos, k)
	, 0.0, 1.0, dur)
