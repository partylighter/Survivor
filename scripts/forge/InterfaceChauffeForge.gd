...
func _rafraichir() -> void:
	jauge.actualiser(gestionnaire_chauffe.temperature_actuelle, gestionnaire_chauffe.obtenir_limite_basse(), gestionnaire_chauffe.obtenir_limite_haute())
	...
	temps_restant.text = "Temps restant : %.1f s" % gestionnaire_chauffe.obtenir_temps_restant()
	progression.value = gestionnaire_chauffe.obtenir_progression_temps() * 100.0
...
func _quand_chauffe_terminee(nouveau_resultat: StringName) -> void:
	...
	set_process(false)
	if gestionnaire_forge != null:
		gestionnaire_forge.terminer_chauffe(nouveau_resultat)
	if nouveau_resultat == GestionnaireChauffe.RESULTAT_ECHEC:
		...
		indication.text = "Etape suivante : Martelage"
		bouton_action.text = "Fermer"
	bouton_action.show()

func _action_resultat() -> void:
	if dernier_resultat == GestionnaireChauffe.RESULTAT_ECHEC:
		ouvrir()
